...
loop do
  break if y >= max_y
  if blocked.includes?(Point.new(x, y+1)) # directly down is blocked
    if blocked.includes?(Point.new(x-1, y+1)) # down and down-left is blocked
      if blocked.includes?(Point.new(x+1, y+1)) # all paths are blocked
